...
class ReachyDashboard:
    ...
    def change_compliancy(self, part: str, compliance: bool):
        ...
        cmd_msg = JointsCommand(commands=[self._build_compliance_cmd_msg(joint, compliance) for joint in flat_joint_list])
    ...
    def set_goal_position(self, joint_goal_pos_dict):
        self.joint_stub.SendJointsCommands(
            JointsCommand(
                commands=[
                    self._build_goal_position_cmd_msg(joint_name, goal_position)
                    for joint_name, goal_position in joint_goal_pos_dict.items()
                ]
            )
        )

    def _build_goal_position_cmd_msg(self, joint_name, goal_position):
        msg = JointCommand(
            id=JointId(name=joint_name),
            compliant=BoolValue(value=True),
            goal_position=FloatValue(value=goal_position),
        )
        return msg
    ...
    async def turn_off_smoothly(self, part: str):
        ...
        cmd_msg = JointsCommand(commands=[self._build_torque_limit_cmd_msg(joint, 0) for joint in flat_joint_list])
        self.joint_stub.SendJointsCommands(cmd_msg)

        await asyncio.sleep(2.0)

        cmd_msg = JointsCommand(commands=[self._build_compliance_cmd_msg(joint, True) for joint in flat_joint_list])
        self.joint_stub.SendJointsCommands(cmd_msg)

        cmd_msg = JointsCommand(commands=[self._build_torque_limit_cmd_msg(joint, 100) for joint in flat_joint_list])
        self.joint_stub.SendJointsCommands(cmd_msg)
    ...
    def get_states(self):
        ...
        for part in self.joints:
            state_part = {}
            ids = [JointId(name=joint) for joint in self.joints[part]]
            request = JointsStateRequest(ids=ids, requested_fields=[JointField.TEMPERATURE, JointField.PRESENT_POSITION])
            res = self.joint_stub.GetJointsState(request)
